    ...
    <v-card>
        ...
        >
            <v-toolbar-title :class="{'subheading': $vuetify.breakpoint.xs}">
                <v-layout>
                    <v-icon class="mr-2">mdi-cup-water</v-icon>
                    <div>
                        Daftar Cara Minum Obat
                    </div>
                </v-layout>
            </v-toolbar-title>
        </v-toolbar>
            ...
            <v-layout column fill-height>
              <v-card-actions class="indigo darken-2">
                  <v-layout wrap row class="headline white--text ml-2">
                    <v-icon dark medium class="mr-2">mdi-cup-water</v-icon>
                    <div>{{ formTitle }}</div>
                  </v-layout>
                  <v-spacer></v-spacer>
  ...
  export default {
    data: () => ({
      ...
      headers: [
        ...
        { text: 'Kode', align: 'left', sortable: true, value: 'kode' },
        { text: 'Cara Minum', align: 'left', sortable: true, value: 'cara' },
    ...
    computed: {
      ...
      formTitle () {
        return this.editedIndex === -1 ? 'New Cara Minum' : 'Edit Cara Minum'
      },
    },
    ...
    methods: {
      ...
      editItem (item) {
        this.editedIndex = this.nginums.indexOf(item)
      ...
      deleteItem (item) {
        ...
        swal.fire({
        title: 'Anda yakin?',
        html: 'Hapus data dari <span class="red--text font-weight-bold">'+item.cara+'</span> <br> Data yang sudah dihapus tidak bisa dikembalikan lagi',
        type: 'warning',
        ...
        confirmButtonText: 'Yes, hapus!'
              ...
              if (response.value) {
                    this.$Progress.start();
                    axios.delete('api/minum-obat/'+item.id).then(()=>{
                      swal.fire(
                      'Deleted!',
                      'Data dari <span class="green--text font-weight-bold">'+item.cara+'</span>  telah terhapus!!.',
                      'success'
                      )
                      setTimeout(()=>{
                        this.initialize()
      ...
      save () {
        ...
        if (this.editedIndex > -1) {
          ...
          if (this.$refs.form.validate()){
            axios.put('api/minum-obat/'+this.editedItem.id, this.editedItem).then((response) => {
                setTimeout(()=>{
                  toast.fire({
                    type: 'success',
                    title: 'Data Cara Minum berhasil diupdate~'
                  })
                  this.initialize()
                  this.$Progress.finish();
                  this.close()
          ...
          this.$Progress.start()
          if(this.$refs.form.validate()){
            axios.post('api/minum-obat', this.editedItem).then((response)=>{
              setTimeout(()=>{
                toast.fire({
                  type: 'success',
                  title: 'Cara Minum berhasil ditambahkan~'
                })
                this.initialize()
                this.$Progress.finish();
                this.close()
