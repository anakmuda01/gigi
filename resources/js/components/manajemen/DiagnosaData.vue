            ...
            <v-layout column fill-height>
              <v-card-actions class="indigo darken-2">
                  <v-layout wrap row class="headline white--text ml-2">
                    <v-icon dark medium class="mr-2">mdi-prescription</v-icon>
                    <div>{{ formTitle }}</div>
                  </v-layout>
                  <v-spacer></v-spacer>
                    ...
                    <v-container class="px-4 pt-3 pb-5" grid-list-md>
                        <v-layout wrap>
                            <v-flex xs12 sm12 md12>
                              <v-text-field 
                              v-model="editedItem.nama_diagnosa" 
                              label="Nama Diagnosa"
                              maxlength="90"
                              counter
          ...
          <template v-slot:items="props">
              <tr>
                ...
                <td class="text-xs-left text-no-wrap"> 
                  {{ props.item.nama_diagnosa | ucWords }}
                </td>
  ...
  export default {
    data: () => ({
      ...
      headers: [
        ...
        { text: 'Nama Diagnosa', align: 'left', sortable: true, value: 'nama_diagnosa' },
    ...
    computed: {
      ...
      formTitle () {
        return this.editedIndex === -1 ? 'New Diagnosa' : 'Edit Diagnosa'
      },
    },
    ...
    methods: {
      ...
      editItem (item) {
        this.editedIndex = this.diags.indexOf(item)
      ...
      deleteItem (item) {
        ...
        swal.fire({
        title: 'Anda yakin?',
        html: 'Hapus data dari <span class="red--text font-weight-bold">'+item.nama_diagnosa+'</span> <br> Data yang sudah dihapus tidak bisa dikembalikan lagi',
        type: 'warning',
        ...
        confirmButtonText: 'Yes, hapus!'
              ...
              if (response.value) {
                    this.$Progress.start();
                    axios.delete('api/diagnosa/'+item.id).then(()=>{
                      swal.fire(
                      'Deleted!',
                      'Data dari <span class="green--text font-weight-bold">'+item.nama_diagnosa+'</span>  telah terhapus!!.',
                      'success'
                      )
                      setTimeout(()=>{
                        this.initialize()
      ...
      save () {
        ...
        if (this.editedIndex > -1) {
          ...
          if (this.$refs.form.validate()){
            axios.put('api/diagnosa/'+this.editedItem.id, this.editedItem).then((response) => {
                setTimeout(()=>{
                  toast.fire({
                    type: 'success',
                    title: 'Data Diagnosa berhasil diupdate~'
                  })
                  this.initialize()
                  this.$Progress.finish();
                  this.close()
              ...
              toast.fire({
                type: 'warning',
                title: 'Maaf, '+error.response.data.errors.nama_diagnosa
              })
              setTimeout(() => {
                this.$Progress.finish();
                this.close()
          ...
          this.$Progress.start()
          if(this.$refs.form.validate()){
            axios.post('api/diagnosa', this.editedItem).then((response)=>{
              setTimeout(()=>{
                toast.fire({
                  type: 'success',
