            ...
            <v-layout column fill-height>
              <v-card-actions class="indigo darken-2">
                  <v-layout wrap row class="headline white--text ml-2">
                    <v-icon dark medium class="mr-2">mdi-pill</v-icon>
                    <div>{{ formTitle }}</div>
                  </v-layout>
                  <v-spacer></v-spacer>
                    ...
                    <v-container class="px-4 pt-3 pb-5" grid-list-md>
                        <v-layout wrap>
                            ...
                            <v-flex xs12 sm12 md12>
                              ...
                              :rules="[rules.required]"
                              :items="['Keping','Botol','Buah','Kaplet']"
                              clearable
                              :disabled="loadingBtn"
                              ></v-select>
                            </v-flex>
  ...
  export default {
    data: () => ({
      ...
      headers: [
        ...
        { text: 'Nama Obat', align: 'left', sortable: true, value: 'nama_obat' },
        { text: 'Satuan', align: 'left', sortable: true, value: 'satuan' },
    ...
    computed: {
      ...
      formTitle () {
        return this.editedIndex === -1 ? 'New Obat' : 'Edit Obat'
      },
    },
    ...
    methods: {
      ...
      editItem (item) {
        this.editedIndex = this.obats.indexOf(item)
      ...
      deleteItem (item) {
        ...
        swal.fire({
        title: 'Anda yakin?',
        html: 'Hapus data dari <span class="red--text font-weight-bold">'+item.nama_obat+'</span> <br> Data yang sudah dihapus tidak bisa dikembalikan lagi',
        type: 'warning',
        ...
        confirmButtonText: 'Yes, hapus!'
              ...
              if (response.value) {
                    this.$Progress.start();
                    axios.delete('api/obat/'+item.id).then(()=>{
                      swal.fire(
                      'Deleted!',
                      'Data dari <span class="green--text font-weight-bold">'+item.nama_obat+'</span>  telah terhapus!!.',
                      'success'
                      )
                      setTimeout(()=>{
                        this.initialize()
      ...
      save () {
        ...
        if (this.editedIndex > -1) {
          ...
          if (this.$refs.form.validate()){
            axios.put('api/obat/'+this.editedItem.id, this.editedItem).then((response) => {
                setTimeout(()=>{
                  toast.fire({
                    type: 'success',
                    title: 'Data Obat berhasil diupdate~'
                  })
                  this.initialize()
                  this.$Progress.finish();
                  this.close()
              ...
              toast.fire({
                type: 'warning',
                title: 'Maaf, '+error.response.data.errors.nama_obat
              })
              setTimeout(() => {
                this.$Progress.finish();
                this.close()
          ...
          this.$Progress.start()
          if(this.$refs.form.validate()){
            axios.post('api/obat', this.editedItem).then((response)=>{
              setTimeout(()=>{
                toast.fire({
                  type: 'success',
