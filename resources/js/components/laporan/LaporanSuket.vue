  ...
  export default {
    data: () => ({
      ...
      headers: [
        ...
        { text: 'Tgl Surat', value: 'tgl_suket', sortable: true },
        { text: 'No. Suket', value: 'no_suket', sortable: true },
        { text: 'Nama Pasien', value: 'agt.nama_pasien', sortable: true },
        { text: 'No RM', value: 'agt.no_rm', sortable: true },
      ],
      mode: 'Perbulan',
      live: null,
      tableShow: false,
    ...
    methods: {
      ...
      CariHistory(){
        ...
        if(this.mode === 'Detail'){
          if(this.$refs.formDetail.validate()){
            axios.get('api/laporan-suket/'+mod+'/'+this.fromDate+'/'+this.toDate).then((response)=>{
          ...
          if(this.$refs.formPerbulan.validate()){
            this.loadingTB = true
            axios.get('api/laporan-suket/'+mod+'/'+this.bulan+'/'+this.tahun).then((response)=>{
              this.history = response.data
              this.tableShow = true
              this.ready()
              this.$Progress.finish()
          ...
          if(this.$refs.formPertahun.validate()){
            this.loadingTB = true
            axios.get('api/laporan-suket/'+mod+'/pertahun/'+this.pertahun).then((response)=>{
              this.history = response.data
              this.tableShow = true
              this.ready()
              this.$Progress.finish()
      ...
      Print(){
        if(this.history.length > 0){
          ...
          doc.setFontSize(16);
          doc.setFontStyle('bold');
          doc.myText("LAPORAN RIWAYAT SURAT KETERANGAN",{align: "center"},0,34);
          doc.setFontSize(13);
          if(this.mode === "Detail"){
          ...
          for(let i=0;i<his.length;i++){
            body.push({
              ...
              tgl: his[i].tgl_surat,
              nosu: his[i].no_suket,
              pasien: his[i].agt.nama_pasien,
              no_rm: his[i].agt.no_rm,
            })
          }
          ...
          doc.autoTable({
            ...
            {header: 'No. Surat', dataKey: 'nosu'},
            {header: 'Nama Pasien', dataKey: 'pasien'},{header: 'No. RM', dataKey: 'no_rm'},
            ],
            bodyStyles: {valign: 'top'},
            headStyles: {fillColor: [48,63,159]},
            styles: {cellWidth: 'wrap', rowPageBreak: 'auto', halign: 'justify'},
            columnStyles: {keluhan: {cellWidth: 'auto'}},
          ...
          if(doc.autoTable.previous.finalY+50 > doc.internal.pageSize.height-10){
            ...
            doc.setFontSize(10);
            doc.setFontStyle('bold');
            doc.text(11, 18, 'Total Surat Keterangan : '+this.history.length)
            doc.setFontSize(10);
            doc.setFontStyle('normal');
            ...
            doc.setFontSize(10);
            doc.setFontStyle('bold');
            doc.text(11, doc.autoTable.previous.finalY+18, 'Total Surat Keterangan : '+this.history.length)
            doc.setFontSize(10);
            doc.setFontStyle('normal');
