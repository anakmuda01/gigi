  ...
  export default {
    data: () => ({
      ...
      headers: [
        ...
        { text: 'Nama Tindakan', value: 'nama_tindakan', sortable: true },
    ...
    methods: {
      ...
      CariHistory(){
        ...
        if(this.mode === 'Detail'){
          if(this.$refs.formDetail.validate()){
            axios.get('api/laporan-tindakan-terbanyak/'+mod+'/'+this.fromDate+'/'+this.toDate).then((response)=>{
          ...
          if(this.$refs.formPerbulan.validate()){
            this.loadingTB = true
            axios.get('api/laporan-tindakan-terbanyak/'+mod+'/'+this.bulan+'/'+this.tahun).then((response)=>{
              this.history = response.data
              this.tableShow = true
              this.ready()
              this.$Progress.finish()
          ...
          if(this.$refs.formPertahun.validate()){
            this.loadingTB = true
            axios.get('api/laporan-tindakan-terbanyak/'+mod+'/pertahun/'+this.pertahun).then((response)=>{
              this.history = response.data
              this.tableShow = true
              this.ready()
              this.$Progress.finish()
      ...
      Print(){
        if(this.history.length > 0){
          ...
          doc.setFontSize(16);
          doc.setFontStyle('bold');
          doc.myText("LAPORAN TINDAKAN TERBANYAK",{align: "center"},0,34);
          doc.setFontSize(13);
          if(this.mode === "Detail"){
          ...
          doc.autoTable({
            startY: 45,
            body: body,
            columns: [{header: 'Rank', dataKey: 'index'},{header: 'Nama Tindakan', dataKey: 'tkn'}, 
