                        ...
                        <v-layout align-center row wrap>
                          ...
                          <v-flex xs11 sm11 md11 lg11 pl-1 style="border-left:1px solid #e1e1e1">
                            <v-layout column>
                              ...
                              <v-flex v-else>
                                <div class="orange--text">Tidak ada Kontrol</div>
                              </v-flex>
                            </v-layout>
                          </v-flex>
                        </v-layout>
    ...
    methods: {
      ...
      printRM(){
          ...
          for (let i = 0; i < rek.length; i++) {
            if(rek[i].gigis.length > 0){
              ...
              for(let j = 0; j < rek[i].gigis.length; j++){
                ...
                let foo = rek[i].gigis[j].no_gigi
                let f_ket = rek[i].gigis[j].ket ? (rek[i].gigis[j].ket.isi_ket !== null ? rek[i].gigis[j].ket.isi_ket : '-' ) : '-'
                // if(foo.length > 2){
                //   foo = foo.split(',')
                //   let bar = ''
        ...
        for (var i = 0; i < body.length; i++) {
          ...
          row['ng'] = {content: row.ng, styles: {halign: 'left'}};
        }
        ...
        doc.autoTable({
            ...
            theme: 'grid',
            styles: {
              rowPageBreak: 'auto', halign: 'left', valign: 'middle'
            },
            headStyles: {
            ...
            columnStyles: {
              index: {cellWidth: 8},
              tgl: {cellWidth: 15},
              ng: {CellWidth: 10, valign: 'top'},
              diag: {CellWidth: 80, valign: 'top'},
              terapi: {cellWidth: 40, valign: 'top'},
              ket: {cellWidth: 40, valign: 'top'}
            },
      ...
      unduhCok(item){
        ...
        if(doc.autoTable.previous.finalY+90 > doc.internal.pageSize.height-10){
            ...
            doc.setFontStyle('normal');
            doc.text(120,doc.autoTable.previous.finalY+60, this.setItem.kota+', '+format(item.sujuk.tgl_rujukan, 'DD MMMM YYYY', {locale: id}))
