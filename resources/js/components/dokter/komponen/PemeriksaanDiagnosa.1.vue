                                ...
                                <template v-for="i in $vuetify.breakpoint.xs || $vuetify.breakpoint.sm ? reg1 : reg1.slice().reverse()">
                                    ...
                                    <v-tooltip v-else top :key="i">
                                        <template v-slot:activator="{ on }">
                                            <v-btn v-on="on"  @click="chooseGigi('1'+i)" icon small>
            ...
            <v-flex>
                <v-card flat>
                    <v-toolbar flat color="white">              
                        <v-dialog scrollable :fullscreen="$vuetify.breakpoint.xs" v-model="dialog" max-width="600px" persistent>
                            <v-card>
                            ...
                            <v-card-text class="px-1 pt-0">
                                <v-stepper v-model="e1">
                                    ...
                                    <v-stepper-content step="1">
                                        ...
                                        :min-height="$vuetify.breakpoint.xs ? '350px' : ''"
                                        >
                                            <v-card-text>
                    ...
                    <template v-slot:items="props">
                        ...
                        <td :class="{'caption': $vuetify.breakpoint.xs}" class="text-xs-left">
                            <v-icon :color="props.item.rujuk ? 'warning' : 'success'">{{props.item.rujuk ? 'check_circle' : 'mdi-close-circle'}}</v-icon>
                        </td>
                        <td @click.stop="" class="justify-center align-center layout px-0">
                            <v-btn 
                            :disabled="loadingBtn" 
                            @click="makeRujuk(props.item)" icon small class="mr-1" color="primary">
                                <v-icon
                                    small
                                >
                                    print
                                ...
                                <v-layout column>
                                    ...
                                    <v-flex xs12>
                                        <v-layout row wrap>
                                            <v-flex xs12>
                                                <v-select 
                                                v-model="rujukItem.foto.nama" 
                                                :items="arrFoto"
                                                label="Foto"
                                                >
                                                </v-select>
                                            </v-flex>
                                            <v-flex 12 v-if="rujukItem.foto.nama == 'Lainnya'">
                                                <v-text-field 
                                                label="Ket. Foto Lainnya"
                                                v-model="rujukItem.foto.ket"
                                                :rules="[rujukItem.foto.nama == 'Lainnya' ? rules.required : '']"
                                                >
                                                </v-text-field>
                                            </v-flex>
                                        </v-layout>
                                    ...
                                    <v-flex x12>
                                        <v-select 
                                        v-model="rujukItem.protesa.nama" 
                                        :items="arrProtesa"
                                        label="Protesa"
                                        >
                                        </v-select>
                                        <v-text-field 
                                        v-if="rujukItem.protesa.nama == 'Lainnya'" 
                                        label="Ket. Protesa Lainnya"
                                        v-model="rujukItem.protesa.ket"
                                        :rules="[rujukItem.protesa.nama == 'Lainnya' ? rules.required : '']"
                                        >
                                        </v-text-field>
                                    </v-flex>
                                </v-layout>
  ...
  export default {
    ...
    data () {
      return {
        ...
        reg1: angka1,
        reg2: angka2,
        arrFoto: ['Peripikal (lokal)', 'Panoramic','Lainnya'],
        arrProtesa: ['Denture Acrylic', 'Denture Valplast','Porcelain/Crown/Bridge','Lainnya'],
        headers: [
          {
            text: '#',
            align: 'center',
    ...
    methods: {
        ...
        pilihGigi(){
            ...
            this.loadingBtn = true
            this.loadingTB = true
            axios.post('/api/tambah-gigi/'+this.solo.id, {nomor: this.editedItem.no_gigi}).then((response) => {
        ...
        AutoNomorSurat(){
            ...
            this.rujuk.no_rujukan = a[2].substr(2,4)+'/'+a[1]+''+a[0]+'/SR-'+b;
            this.rujuk.tgl_rujukan = format(now, 'DD MMMM YYYY', {locale: id})
        ...
        makeRujuk(item){
            if(item.rujuk){ 
                ...
                if(this.arrFoto.includes(item.rujuk.foto)){
                    this.rujukItem.foto = {
                        nama : item.rujuk.foto,
                        ket : ''
                    }
                }else{
                    this.rujukItem.foto = {
                ...
                if(this.arrProtesa.includes(item.rujuk.protesa)){
                    this.rujukItem.protesa = {
                        nama : item.rujuk.protesa,
                        ket : ''
                    }
                }else{
                    this.rujukItem.protesa = {
                        nama : 'Lainnya',
                        ket : item.rujuk.protesa
                    }
                }
            }
        ...
        cetakSujuk(){
            ...
            doc.setFontSize(13);
            doc.setFontStyle('normal');
            doc.myText('Nomor : '+this.rujuk.no_rujukan,{align: "center"},0,42);
            ...
            // doc.text(55,165, ': '+item.anggota.nama_pasien)
            // doc.text(15,170, 'Umur ')
            // doc.text(55,170, ': '+son.umur(item.anggota.tanggal_lahir))
            // doc.text(15,175, 'Jenis Kelamin ')
            // doc.text(55,175, ': '+son.kelamin(item.anggota.jenis_kelamin))
            // doc.text(15,180, 'No. HP ')
            // doc.text(55,180, ': '+item.anggota.no_hp)
            ...
            // doc.setFontStyle('italic');
            // doc.text(35,230, this.rujuk.mohon)

            // doc.setFontSize(13);
            // doc.setFontStyle('normal');
            // doc.text(140,225, 'Banjarbaru, '+this.rujuk.tgl_rujukan)
